...
<form action="{{ route('admin.projects.update', ['project' => $project->id]) }}" method="POST" class="py-5" novalidate>
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control @error('image_url') is-invalid @enderror" id="image_url" name="image_url" value="{{ $project->image_url }}">
        @error('image_url')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="d-flex justify-content-start">
        ...
        <div class="p-3">
            <a href="{{ route('admin.projects.index', ['project' => $project->id]) }}" class="btn btn-secondary p-2">Return to the
                project</a>
        </div>
    </div>
</form>
